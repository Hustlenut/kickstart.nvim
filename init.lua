...
require('lazy').setup({
  ...
  },
  {
    ...
    config = function()
      ...
      local ok, spring_boot = pcall(require, 'spring-boot')
      if ok then
        spring_boot.setup {}
      end
      ...
      local servers = {
        ...
        perlnavigator = {},
        jdtls = {
          -- Helps JDTLS find the 'root' of your Maven/Gradle project
          root_dir = require('lspconfig.util').root_pattern('pom.xml', 'build.gradle', '.git'),
          cmd = {
            'jdtls',
            string.format('--jvm-arg=-javaagent:%s/packages/jdtls/lombok.jar', vim.fn.stdpath 'data' .. '/mason'),
          },
        },
        -- pyright = {},
        -- rust_analyzer = {},
	...
	'sts4',
      ...
      for i, name in ipairs(ensure_installed) do
        if name == 'sts4' then
          ensure_installed[i] = 'vscode-spring-boot-tools'
        end
      end
...

  {
    'catppuccin/nvim',
    name = 'catppuccin',
    priority = 1000, -- Load this first so it's available for the UI
    config = function()
      require('catppuccin').setup({
        ...
        integrations = {
          ...
          -- This is important for Spring/Java setup
          native_lsp = {
            enabled = true,
            virtual_text = {
              errors = { 'italic' },
